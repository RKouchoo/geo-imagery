...
datDirs = SubDirPath(Path(r'../../data/processed/noaa-goes18'))
...
for dataDir in datDirs:
    ...
    imageName = f"{renderTemplate}.{str(dataDir).strip("data/processed/noaa-goes18/")}.png"
    print(imageName)
...
for job in renderQue:
    ...
    with dask.config.set({"array.chunk-size" : "512MiB"}):
        ...
        imageName = f"{renderTemplate}.{str(job).strip("data/processed/noaa-goes18/")}.png"

        if outputdir + imageName in existingImgs:
            print(f"Image found, skipping {imageName}")
            continue

        ahi_dataset_reader = find_files_and_readers(base_dir=job, reader="abi_l1b")
        dataset_scene = Scene(reader="abi_l1b", filenames=ahi_dataset_reader, reader_kwargs={'mask_space': False})
